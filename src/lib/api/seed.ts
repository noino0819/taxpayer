...
async function seedStocks(classroomId: string) {
  ...
  const { data: inserted } = await supabase.from('stocks').insert(stocks).select('id, current_price')

  if (inserted && inserted.length > 0) {
    const historyEntries = inserted.map((s) => ({
      stock_id: s.id,
      price: s.current_price,
      previous_price: s.current_price,
      change_reason: 'manual',
    }))
    await supabase.from('stock_price_history').insert(historyEntries)
  }
}
